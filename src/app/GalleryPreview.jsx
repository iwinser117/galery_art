...
const GalleryPreview = () => {
  ...
  return (
    <section ref={sectionRef} className="container mx-auto px-4 py-12 opacity-0 transition-opacity duration-1000 h-full">
      <h2 className="text-3xl md:text-4xl font-bold text-center mb-8">Destacados</h2>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {images.map((image, index) => (
          <div key={index} className="relative group overflow-hidden rounded-lg shadow-lg h-full">
            <Image
              src={image.src}
              alt={image.alt}
              width={1100}
